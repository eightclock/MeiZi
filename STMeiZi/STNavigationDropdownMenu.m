...
@interface STNavigationDropdownMenu ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation STNavigationDropdownMenu

- (instancetype)initWithNavigationController:(UINavigationController *)navigationController{
    
    self = [STNavigationDropdownMenu buttonWithType:UIButtonTypeCustom];
...
- (void)layoutSubviews{
    ...
    [self setTitleEdgeInsets:UIEdgeInsetsMake(0.0,-CGRectGetWidth(self.imageView.frame), 0, CGRectGetWidth(self.imageView.frame) + self.arrowPadding )];
    [self setImageEdgeInsets:UIEdgeInsetsMake(0.0, CGRectGetWidth(self.titleLabel.frame) + self.arrowPadding, 0, -CGRectGetWidth(self.titleLabel.frame))];
...
- (void) scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    CGRect menuHeadViewFrame = self.menuHeaderView.frame;
    if(scrollView.contentOffset.y < 0){
        menuHeadViewFrame.size.height = - scrollView.contentOffset.y;
    }else{
        menuHeadViewFrame.size.height = 0.0;
    }
    
    self.menuHeaderView.frame = menuHeadViewFrame;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width * 2, 0);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([self.delegate respondsToSelector:@selector(navigationDropdownMenu:didSelectedAtIndex:)]){
        [self.delegate navigationDropdownMenu:self didSelectedAtIndex:indexPath.row];
...
- (void)menuAction:(STNavigationDropdownMenu *)sender{
...
//        }
      //  NSLog(@"cellLayoutMarginsFollowReadableWidth = %d",_menuTableView.cellLayoutMarginsFollowReadableWidth);
#warning why register class
...
- (void)setDataSource:(id<STNavigationDropdownMenuDataSource>)dataSource{
    _dataSource = dataSource;
    [self setTitle:self.titleArray.firstObject forState:UIControlStateNormal];
}

- (void)setDelegate:(id<STNavigationDropdownMenuDelegate>)delegate{
    _delegate = delegate;
    if([delegate respondsToSelector:@selector(navigationDropdownMenu:didSelectedAtIndex:)]){
...
- (CGFloat) arrowPadding{
    ...
    if([self.dataSource respondsToSelector:@selector(arrowPaddingForNavigationDropdownMenu:)]){
